...
class NutrientTrackingService {
  ...
  static Future<Map<String, dynamic>> getDailyTracking({String? date}) async {
    try {
      ...
      final uri = Uri.parse(
        '$baseUrl/nutrients/tracking/daily${date != null ? '?date=$date' : ''}',
      );
  ...
  static Future<Map<String, dynamic>> getNutrientBreakdown({
    ...
    try {
      final headers = await _getHeaders();
      final uri = Uri.parse(
        '$baseUrl/nutrients/tracking/breakdown${date != null ? '?date=$date' : ''}',
      );
  ...
  static Future<Map<String, dynamic>> getSummary({String? date}) async {
    try {
      final headers = await _getHeaders();
      final uri = Uri.parse(
        '$baseUrl/nutrients/tracking/summary${date != null ? '?date=$date' : ''}',
      );
  ...
  static Future<Map<String, dynamic>> getComprehensiveReport({
    ...
    try {
      final headers = await _getHeaders();
      final uri = Uri.parse(
        '$baseUrl/nutrients/tracking/report${date != null ? '?date=$date' : ''}',
      );

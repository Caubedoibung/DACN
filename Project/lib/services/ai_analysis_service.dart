...
class AiAnalysisService {
  ...
  static Map<String, dynamic> _parseItem(Map<String, dynamic> json) {
    return {
      ...
      'confidence_score': (json['confidence_score'] ?? 0).toDouble(),
      'estimated_volume_ml': (json['estimated_volume_ml'] ?? 0).toDouble(),
      'estimated_weight_g': (json['estimated_weight_g'] ?? 0).toDouble(),
      'water_ml': (json['water_ml'] ?? 0).toDouble(),
  ...
  static Future<bool> acceptAnalysis(int mealId) async {
    final token = await AuthService.getToken();
    if (token == null) throw Exception('Chưa đăng nhập');

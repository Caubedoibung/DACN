...
class ChatService {
  ...
  static Future<List<Map<String, dynamic>>> getChatbotMessages(int conversationId) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString('auth_token') ?? prefs.getString('token');
      ...
      final response = await http.get(
        Uri.parse('$baseUrl/chat/chatbot/conversation/$conversationId/messages'),
        headers: {'Authorization': 'Bearer $token'},
      );
  ...
  static Future<Map<String, dynamic>?> analyzeFoodImage(
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('$baseUrl/chat/chatbot/conversation/$conversationId/analyze-image'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
        },
  ...
  static Future<List<Map<String, dynamic>>> getAdminMessages(int conversationId) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString('auth_token') ?? prefs.getString('token');
      ...
      final response = await http.get(
        Uri.parse('$baseUrl/chat/admin-chat/conversation/$conversationId/messages'),
        headers: {'Authorization': 'Bearer $token'},
      );
  ...
  static Future<Map<String, dynamic>?> sendAdminMessage(
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('$baseUrl/chat/admin-chat/conversation/$conversationId/message'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
        },

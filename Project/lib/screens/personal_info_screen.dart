...
class _PersonalInfoScreenState extends State<PersonalInfoScreen> {
  ...
  void _onTruncateDetected(String fieldLabel) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('$fieldLabel quá dài, đã cắt xuống giới hạn')),
    );
  }
  ...
  String? _decimalValidator(
    ...
    if ('.'.allMatches(s).length > 1) return '$fieldName không hợp lệ';
    final parts = s.split('.');
    final intPart = parts[0];
    final fracPart = parts.length > 1 ? parts[1] : '';
    if (intPart.length > maxInteger) {
      return '$fieldName tối đa $maxInteger chữ số phần nguyên';
    }
    if (fracPart.length > maxFraction) {
      return '$fieldName tối đa $maxFraction chữ số thập phân';
    }
    final val = double.tryParse(s.replaceAll(',', '.'));
    if (val == null) return '$fieldName phải là số';
    if (min != null && val < min) return '$fieldName phải >= $min';
    if (max != null && val > max) return '$fieldName phải <= $max';
    return null;
  }
                          ...
                          Center(
                            child: Column(
                              children: [
                                ...
                                TextButton(
                                  onPressed: _pickAvatar,
                                  child: const Text('Thay d?i ?nh d?i di?n'),
                                ),
                              ],
                            ),
                          ),
                              ...
                              Expanded(
                                child: _buildModernTextField(
                                  ...
                                  validator: (v) {
                                    ...
                                    if (n < 5 || n > 120) return 'Tuổi: 5-120';
                                    return null;
                                  },
                                ),
                              ),
                                        ...
                                        Container(
                                          ...
                                          child: Row(
                                            children: [
                                              ...
                                              const SizedBox(width: 8),
                                              Text(
                                                'T?ng: ${proteinPctLocal + fatPctLocal + carbPctLocal}%',
                                                style: TextStyle(
                                                  fontSize: 14,
                                                  fontWeight: FontWeight.bold,
                                                  color:
                            ...
                            onPressed: () async {
                              ...
                              if (s < 99 || s > 101) {
                                ScaffoldMessenger.of(ctx).showSnackBar(
                                  SnackBar(
                                    content: const Text(
                                      'Tổng % Protein+Fat+Carb nên xấp xỉ 100%',
                                    ),
      ...
      try {
        ...
        if (settingsPayload.isNotEmpty) {
          ...
          if (sresp == null || sresp['error'] != null) {
            ...
            messenger.showSnackBar(
              const SnackBar(
                content: Text('Luu c�i d?t dinh du?ng kh�ng th�nh c�ng'),
              ),
            );
          } else {
  ...
  Widget build(BuildContext context) {
    ...
    String genderLabel(dynamic v) {
      ...
      if (s == 'male' || s.startsWith('m')) return 'Nam';
      if (s == 'female' || s.startsWith('f')) return 'Nữ';
      return 'Khác';
    }

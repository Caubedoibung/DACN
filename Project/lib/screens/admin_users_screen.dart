...
class _AdminUsersScreenState extends State<AdminUsersScreen>
  ...
  Future<void> _blockUser(int userId) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(labelText: 'Lý do chặn (tùy chọn)'),
        ),
        actions: [
          TextButton(
    ...
    try {
      ...
      if (resp.statusCode == 200) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Chặn thất bại')));
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(
  ...
  Future<void> _unblockUser(int userId) async {
    try {
      ...
      if (resp.statusCode == 200) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Gỡ chặn thất bại')));
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(
  ...
  String _fmt(String? iso) {
    if (iso == null || iso.isEmpty) return 'Chưa đăng nhập';
    try {
                ...
                child: Column(
                  ...
                  children: [
                    _buildSection('Thông tin cá nhân', [
                      ...
                      _buildInfoRow('Tuổi', '${user['age'] ?? 'N/A'}'),
                    ...
                    _buildSection('Mục tiêu', [
                      _buildInfoRow('Loại chế độ', user['diet_type'] ?? 'N/A'),
                    ...
                    ...recentMeals.map(
                      (meal) => ListTile(
                        ...
                        subtitle: Text(meal['meal_date'] ?? ''),
                        trailing: Text('${meal['total_calories'] ?? 0} kcal'),
                      ),
                    ),

...
async function createWaterEntry(userId, amountMl, date, options = {}) {
  // If no date provided, use current date in Vietnam timezone (UTC+7)
  const client = await db.pool.connect();
  try {
    await client.query("BEGIN");

    // Get the date to use (either provided or current date in Vietnam timezone)
    let isoDate;
    if (date) {
...
async function getWaterTimeline(userId, targetDate) {
  const dateResult = targetDate
    ...
    : await db.query("SELECT CURRENT_DATE::text as date");

-- ============================================
-- COMPLETE DRINK SYSTEM SETUP
-- Tổng hợp: Food + FoodNutrient + DrinkIngredient + Auto Calculate Functions
-- Database: Health/PostgreSQL 18
-- ============================================

-- ============================================
-- PHẦN 0: XÓA DỮ LIỆU MẪU CŨ (NẾU CÓ)
-- ============================================

-- Xóa drinknutrient của các drink có sử dụng food_id từ 88-112
DELETE FROM drinknutrient 
WHERE drink_id IN (
    SELECT DISTINCT drink_id 
    FROM drinkingredient 
    WHERE food_id BETWEEN 88 AND 112
);

-- Xóa drinkingredient có food_id từ 88-112
DELETE FROM drinkingredient WHERE food_id BETWEEN 88 AND 112;

-- Xóa foodnutrient của food_id từ 88-112

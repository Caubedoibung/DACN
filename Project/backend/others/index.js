...
// Thống kê thực phẩm
const { getFoodStats, getFoods } = require("../controllers/foodsStats");
const foodsStatsRouter = require("express").Router();
foodsStatsRouter.get("/stats", getFoodStats);
foodsStatsRouter.get("/", getFoods);
app.use("/api/foods", foodsStatsRouter);

// Thống kê món ăn
const { getDishCategories } = require("../controllers/dishesStats");
const dishesStatsRouter = require("express").Router();
dishesStatsRouter.get("/categories", getDishCategories);
app.use("/api/dishes", dishesStatsRouter);

// Thống kê đồ uống
const { getDrinkCategories, getDrinks } = require("../controllers/drinksStats");
const drinksStatsRouter = require("express").Router();
drinksStatsRouter.get("/categories", getDrinkCategories);
drinksStatsRouter.get("/", getDrinks); // Thêm route trả về danh sách đồ uống
app.use("/api/drinks", drinksStatsRouter);

// Thống kê chất dinh dưỡng
const { getNutrients } = require("../controllers/nutrientsStats");
const nutrientsStatsRouter = require("express").Router();
nutrientsStatsRouter.get("/", getNutrients);
app.use("/api/nutrients", nutrientsStatsRouter);

// Thống kê bệnh lý
const { getHealthConditions } = require("../controllers/healthConditionsStats");
const healthConditionsStatsRouter = require("express").Router();
healthConditionsStatsRouter.get("/", getHealthConditions);
app.use("/api/health-conditions", healthConditionsStatsRouter);

// Thống kê thuốc
const { getDrugs } = require("../controllers/drugsStats");
const drugsStatsRouter = require("express").Router();
drugsStatsRouter.get("/", getDrugs);
app.use("/api/drugs", drugsStatsRouter);
...
async function ensureManualNutritionLogTable() {
  try {
    ...
    await db.query(`
      CREATE UNIQUE INDEX IF NOT EXISTS ux_manual_nutrient_user_date_nutrient
      ON UserNutrientManualLog(user_id, log_date, nutrient_id)
    `);
...
async function ensureHealthAndMedicationTables() {
  try {
    ...
    if (checkMedicationLog.rows.length === 2) {
      await db.query(
        `CREATE INDEX IF NOT EXISTS idx_medicationlog_user_date ON medicationlog(user_medication_id, medication_date)`
      );
    }

...
exports.getDrugsForCondition = async (req, res) => {
  try {
    ...
    // Verify user has this condition
    ...
    if (conditionCheck.rows.length === 0) {
      return res
        .status(404)
        .json({ error: "Condition not found or not active" });
    }

    // Get drugs using the function
    const drugsResult = await db.query(
      `SELECT * FROM get_drugs_for_condition($1)`,
      [conditionId]
    );
...
exports.logMedication = async (req, res) => {
  try {
    const userId = req.user.user_id;
    const { drug_id, user_condition_id, medication_date, medication_time } =
      req.body;
    ...
    // Verify user owns this condition
    const conditionCheck = await db.query(
      `SELECT user_id FROM userhealthcondition 
...
exports.checkInteraction = async (req, res) => {
  try {
    ...
    if (!meal_time) {
      return res.status(400).json({ error: "meal_time is required" });
    }

    const mealTimestamp = new Date(meal_time);
    const foodIdsArray = food_ids
      ? food_ids.split(",").map((id) => parseInt(id))
      : null;
    ...
    return res.json({
      success: true,
      has_interaction: interactionsResult.rows.length > 0,
...
exports.getMedicationHistoryStats = async (req, res) => {
  try {
    ...
    const statsResult = await db.query(
      `SELECT * FROM get_medication_history_stats($1, $2, $3)`,
    ...
    return res
      .status(500)
      .json({ error: "Failed to get medication history stats" });
  }
};
...
exports.getAllDrugs = async (req, res) => {
  try {
    ...
    const conditionsResult = await db.query(
      `SELECT condition_id FROM userhealthcondition 
       WHERE user_id = $1 AND status = 'active'`,
      [userId]
    );
    const userConditionIds = conditionsResult.rows.map(
    ...
    query += ` GROUP BY d.drug_id ORDER BY d.name_vi ASC`;

    const result = await db.query(query, params);

    // Mark which drugs are suitable for user's conditions
    const drugs = result.rows.map((drug) => {
      const drugConditionIds = (drug.conditions || [])
        .map((c) => c["condition_id"])
        .filter((id) => id != null);

      const isSuitable =
        userConditionIds.length > 0 &&
        drugConditionIds.some((id) => userConditionIds.includes(id));

      return {
        ...drug,
        is_suitable_for_user: isSuitable,
        has_any_condition: drugConditionIds.length > 0,
      };
    });
    ...
    return res.json({
      success: true,
      drugs,
      user_condition_ids: userConditionIds,
    });
  } catch (error) {
...
// GET /api/medications/schedule - Get user's medication schedule
exports.getMedicationSchedule = async (req, res) => {
  try {
    ...
    // Get all dates that have medication schedules
    ...
    // Get medication logs for the date to check taken status
    const logsResult = await db.query(
      `SELECT 
        ml.log_id,
        ml.user_condition_id,
        ml.medication_date,
        ml.medication_time::TEXT,
...
exports.getTodayMedication = async (req, res) => {
  try {
    ...
    // Get all medication times for today from user conditions
    ...
    // Get logs for today to check taken status
    const logsResult = await db.query(
      `SELECT 
        ml.user_condition_id,
        ml.medication_time::TEXT,
    ...
    // Merge schedule with logs
    const medications = scheduleResult.rows.map((schedule) => {
      const log = logsResult.rows.find(
        (l) =>
          l.user_condition_id === schedule.user_condition_id &&
          l.medication_time === schedule.medication_time
      );
      return {
...
exports.getMedicationStatistics = async (req, res) => {
  try {
    ...
    // Calculate 30 days ago in Vietnam timezone
    ...
    // Get statistics with 1-hour window (1 hour = 3600 seconds)
    const result = await db.query(
      `SELECT 
        COUNT(*) as total_doses,
        COUNT(CASE WHEN ml.status = 'taken' THEN 1 END) as taken_doses,
        COUNT(CASE WHEN ml.status = 'taken' 
          AND ABS(EXTRACT(EPOCH FROM (ml.taken_at::time - ml.medication_time::time))) <= 3600 
          THEN 1 END) as on_time_doses,
        COUNT(CASE WHEN ml.status = 'taken' 
          AND ABS(EXTRACT(EPOCH FROM (ml.taken_at::time - ml.medication_time::time))) > 3600 
          THEN 1 END) as late_doses,
    ...
    const adherence_rate = stats.total_doses > 0 
      ? ((parseInt(stats.taken_doses) / parseInt(stats.total_doses)) * 100).toFixed(1)
      : 0;

    const on_time_rate = stats.taken_doses > 0
      ? ((parseInt(stats.on_time_doses) / parseInt(stats.taken_doses)) * 100).toFixed(1)
      : 0;

    return res.json({
      success: true,
      statistics: {
        total_doses: parseInt(stats.total_doses),
        taken_doses: parseInt(stats.taken_doses),
        on_time_doses: parseInt(stats.on_time_doses),
        late_doses: parseInt(stats.late_doses),
        missed_doses: parseInt(stats.missed_doses),

...
async function me(req, res) {
  ...
  if (req.user) {
    try {
      ...
      // attach today's aggregated totals (if any)
      try {
        const db = require("../db");
        const ds = await db.query(
          "SELECT total_calories, total_protein, total_fat, total_carbs, total_water FROM DailySummary WHERE user_id = $1 AND date = (CURRENT_TIMESTAMP AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Ho_Chi_Minh')::date LIMIT 1",
          [user.user_id]
        );
        ...
        try {
          const last = await db.query(
            "SELECT created_at FROM WaterLog WHERE user_id = $1 AND log_date = (CURRENT_TIMESTAMP AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Ho_Chi_Minh')::date ORDER BY created_at DESC LIMIT 1",
            [user.user_id]
          );
  ...
  try {
    ...
    try {
      ...
      await transporter.sendMail({
        from: process.env.SMTP_FROM || "no-reply@mydiary.local",
        to: row.email,
        subject: "Mã đổi mật khẩu",
        text: `Mã xác nhận đổi mật khẩu của bạn là: ${code}. Mã hết hạn sau 10 phút.`,
      });
    } catch (e) {

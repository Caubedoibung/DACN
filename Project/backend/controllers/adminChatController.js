...
exports.getOrCreateConversation = async (req, res) => {
  try {
    ...
    let result = await db.query(
      `SELECT admin_conversation_id, status, subject, created_at, updated_at 
    ...
    if (result.rows.length === 0) {
      ...
      result = await db.query(
        `INSERT INTO AdminConversation (user_id, subject, status) 
         ...
         RETURNING admin_conversation_id, status, subject, created_at, updated_at`,
        [userId]
      );
    }
...
exports.getMessages = async (req, res) => {
  try {
    ...
    const result = await db.query(
      `SELECT admin_message_id, sender_type, sender_id, message_text, image_url, is_read, created_at
       FROM AdminMessage
  ...
  try {
    ...
    const result = await db.query(
      `INSERT INTO AdminMessage (admin_conversation_id, sender_type, sender_id, message_text, image_url)
       VALUES ($1, 'user', $2, $3, $4)
       RETURNING admin_message_id, sender_type, sender_id, message_text, image_url, is_read, created_at`,
